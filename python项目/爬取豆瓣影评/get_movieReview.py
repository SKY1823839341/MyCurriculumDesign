...
excel_name = 'D:/douban_hot_review.xls'
sheet_name = '豆瓣影评'
column = ['标题', '作者', '影片', '影评']
...
def get_movie_review():
    ...
    for row in range(0, len(list_title)):
        if row == 0:
            ...
            print
            '标题：', html_parser(list_title[row])
            print
            '作者：', html_parser(list_people[row])
            print
            '影片：', html_parser(list_subject[row])
            print
            '影评：', html_parser(list_description[row])
            print
            '----------获取第' + str(row + 1) + '个影评结束----------'
        else:
            print
