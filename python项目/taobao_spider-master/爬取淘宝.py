...
class Taobao_Comments:
    def __init__(self):
        ...
        self.read_path = 'taobao_search.csv'
    ...
    def parse_tmall_comments(self, json_content, url, item_id, seller_id, title, product_pic, headers, params):
        ...
        for comments in comments_list:
            ...
            result = [url, item_id, seller_id, title, product_pic, username, comments_id, content, pic_result,
                      comments_date]
    ...
    def parse_taobao(self, url, detail_id, user_id, title, product_pic):
        ...
        max_page = self.parse_taobao_comments(json_content, url, detail_id, user_id, title, product_pic, headers,
                                              params)
        if max_page > 1:
            time.sleep(self.sleep_time)
    ...
    def parse_taobao_comments(self, json_content, url, detail_id, user_id, title, product_pic, headers, params):
        ...
        for comments in comments_list:
            ...
            if len(comments['photos']) == 0:
                ...
                for pic in comments['photos']:
                    ...
                    with open(save_path_pic.rstrip('/') + '/' + ranstr + pic['url'][-4:].replace('/', ''), 'wb') as f:
                        img_get = requests.get('https:' + pic['url'], params=params, headers=headers,
                                               timeout=10).content
                        f.write(img_get)
                    ###########
            ...
            result = [url, detail_id, user_id, title, product_pic, username, comments_id, content, pic_result,
                      comments_date]
            self.save_to_csv(result)
